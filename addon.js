...
const CATALOG_CACHE_MAX_AGE = 1300;
const CATALOG_STALE_REVALIDATE = 60 * 2;
const CATALOG_STALE_ERROR = 60 * 60 * 24;
...
const tmdbEndpointMap = {
    // Nessun catalogo configurato (K-Drama rimosso)
};
...
builder.defineCatalogHandler(async ({ type, id, extra }) => {
    ...
    if (searchQuery) {
        console.warn(`La ricerca per il tipo '${type}' non è supportata perché non ci sono cataloghi definiti.`);
        ...
        console.warn(`Nessun catalogo configurato per la richiesta: type=${type}, id=${id}.`);
        return Promise.resolve({
            metas: [],
            hasMore: false,
            cacheMaxAge: CATALOG_CACHE_MAX_AGE,
